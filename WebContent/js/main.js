...
var GREEN_PLAYABLE_CELLS = new Array;
...
var NEAREST_CELL = new Array;
var CURRENT_PAWN_POS_BEFORE_MOUSEDOWN = new Object();
...
function init() {
    ...
    if (canvas.getContext) {
        ...
        GREEN_PLAYABLE_CELLS = PLAYABLE_CELLS;
        GREEN_PLAYABLE_CELLS.pop(); //Avoid to make a full turn
...
//				console.log(Math.pow(PAWN_RADIUS,2));
                ...
                {
                    TARGET_PAWN = i;
                    CURRENT_PAWN_POS_BEFORE_MOUSEDOWN.X = circleX;
                    CURRENT_PAWN_POS_BEFORE_MOUSEDOWN.Y = circleY;
                    break;
                }
            }
        });
...
//				var mouseY = e.layerY || 0;
                ...
                for (i = 0; i < PLAYABLE_CELLS.length; i++)
                {
                    ...
                    if(Math.pow(mouseX - PLAYABLE_CELLS[i].X, 2) + Math.pow(mouseY - PLAYABLE_CELLS[i].Y, 2) < Math.pow(PAWN_RADIUS, 2)){
                        //console.log(PLAYABLE_CELLS[i]);
                        NEAREST_CELL = PLAYABLE_CELLS[i]; 
                        //TODO : RESTRICTION TO THE PAWN PLAYABLE CELLS
                    }
                }
        ...
        $("#scene").mouseup(function(e) {
            alert("mouseUp");
            console.log(NEAREST_CELL);
            console.log(CURRENT_PAWN_POS_BEFORE_MOUSEDOWN);
            if(TARGET_PAWN && NEAREST_CELL.X ){
            PAWNS[TARGET_PAWN] = new Pawn(PAWNS[TARGET_PAWN].color, new Coordinates(NEAREST_CELL.X, NEAREST_CELL.Y));
            }else{
                if(TARGET_PAWN ) PAWNS[TARGET_PAWN] = new Pawn(PAWNS[TARGET_PAWN].color, new Coordinates(CURRENT_PAWN_POS_BEFORE_MOUSEDOWN.X, CURRENT_PAWN_POS_BEFORE_MOUSEDOWN.Y));
            }
